...
from .report import humanize_amount
...
<div class="sub">Generated {ts} | block <code>{block_str}</code> | {n_markets} markets | {n_borrowers} borrowers analyzed ({coverage_str} of market debt)</div>
...
<div class="sub" style="margin-top:-14px">Supply / borrow figures below are <b>market-wide</b>: a MetaMorpho vault supplies into shared Morpho Blue markets, so borrower positions span all users of those markets, not only debt funded by this vault (which is why market debt can exceed the vault's own assets).</div>
...
def as_html(snapshot: VaultSnapshot, results: list | None = None) -> str:
    ...
    total_assets_human = humanize_amount(snapshot.total_assets, dec, sym)
    block_str = f"{snapshot.block:,}" if snapshot.block else "n/a (not block-pinned)"
    ...
    coverage_str = (
        f"{borrower_debt / market_borrow:.1%}" if market_borrow else "n/a"
    )
    ...
    for m in snapshot.markets:
        ...
        market_rows_html.append(
            ...
            f"<td style='text-align:right'>{humanize_amount(m.total_supply_assets, dec, sym)}</td>"
            f"<td style='text-align:right'>{humanize_amount(m.total_borrow_assets, dec, sym)}</td>"
            f"<td style='text-align:right'{util_class}>{m.utilization:.1%}</td>"
            f"<td style='text-align:right'>{m.lltv:.0%}</td>"
            f"<td style='text-align:right'>{humanize_amount(m.supply_cap, dec, sym)}</td></tr>"
        )
