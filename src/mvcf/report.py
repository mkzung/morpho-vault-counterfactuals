...
def as_markdown(snapshot: VaultSnapshot, results: list[DetectorResult]) -> str:
    ...
    lines.append(f"- **HHI (depositor concentration):** `{snapshot.hhi:.3f}` "
    ...
    lines.append("")
    lines.append("- If `OracleFreezeReplay` headline > 5% — review oracle update cadence + "
                 "consider Chainlink fallback or oracle-router timeout.")
    lines.append("- If `CollateralCascade` headline > 30% at −20% shock — lower LLTV on the "
                 "concentrated market(s) by 1-3 percentage points.")
    lines.append("- If `DepositorExitShock` headline > 20% rationing — reduce vault-level "
                 "supply cap to that market to free idle supply.")
    lines.append("- If `UtilizationInversion` flags ≥ 50% of markets — raise IRM upper "
                 "rate or reallocate via the MetaMorpho `reallocate()` flow.")
    lines.append("- If `LiquidationLatency` headline > 5% — raise the minimum-position-size "
                 "threshold (cap supply on markets where dust positions accumulate).")
    lines.append("- If `LTVDistributionStress` shows >40% of debt within 5pp of LLTV — "
                 "lower LLTV proactively before the next adverse oracle move.")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("*Generated by [morpho-vault-counterfactuals]"
